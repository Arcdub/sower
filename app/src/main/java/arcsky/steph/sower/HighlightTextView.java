 ...
 * (Layout.getSelectionPath): tight per-line stripes with softened corners that
 * skip the added line spacing. A committed highlight therefore looks exactly
 * like the selection did while the finger was still down, instead of the solid
 * full-line-height block a BackgroundColorSpan paints.
 */
public class HighlightTextView extends androidx.appcompat.widget.AppCompatTextView {
    ...
    public HighlightTextView(Context context, AttributeSet attrs) {
        ...
        CornerPathEffect corners =
                new CornerPathEffect(getResources().getDisplayMetrics().density * 4);
        gold.setColor(0x59C8A24B);
        gold.setPathEffect(corners);
        green.setColor(0x408CBF94);
        green.setPathEffect(corners);
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (layout != null && (!highlights.isEmpty() || jumpStart >= 0)) {
            ...
            if (jumpStart >= 0 && jumpEnd > jumpStart && jumpEnd <= length) {
                path.reset();
                layout.getSelectionPath(jumpStart, jumpEnd, path);
                canvas.drawPath(path, green);
            }
            for (int[] r : highlights) {
                if (r[0] >= 0 && r[1] > r[0] && r[1] <= length) {
                    path.reset();
                    layout.getSelectionPath(r[0], r[1], path);
